  ...
  <div class="company-msg box-padding customer-manage">
    ...
    <div class="table-contair">
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <!-- <el-table-column prop="introduce" label="内容介绍"></el-table-column> -->
        ...
        <el-table-column fixed="right" label="操作" width="160">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark"  v-if="scope.row.isStick == 0" content="置顶" placement="top-start">
              <el-button
                circle
                size="mini"
                type="success"
              ...
              <el-tooltip class="item" effect="dark" v-if="scope.row.isStick == 1" content="取消置顶" placement="top-start">
              <el-button
                circle
                size="mini"
                type="danger"
            ...
            <el-tooltip class="item" effect="dark" v-if="scope.row.isRecommend == 0" content="推荐" placement="top-start">
              <el-button
                circle
                size="mini"
                type="info"
              ...
              ></el-button>
               ...
               <el-tooltip class="item" effect="dark" v-if="scope.row.isRecommend == 1" content="取消推荐" placement="top-start">
              <el-button
                circle
                size="mini"
                type="danger"
      ...
      <el-form :model="viewData" label-width="100px">
        <el-form-item label="内容">
           <div class="edit_container">
            <quill-editor v-model="viewData.introduce"
                    ref="myQuillEditor"
                    class="editer"
        ...
        <!-- <el-input
            ...
            :rows="5"
            placeholder="请输入内容"
            v-model="viewData.introduce" autocomplete="off">
          </el-input> -->
      </el-form-item> 
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        let msg = "是否将该条信息置顶？";
        let obj = {"id":row.id ,"isStick":1};
        if(row.isStick==1){
               msg = "是否取消该条信息置顶？";
               obj = {"id":row.id ,"isStick":0};
        }
        self.messageBox(msg,()=>{
          debugger
          fetch("/supply/update",obj, "PUT").then(
            res => {
              self.loadData();
              self.$notify({
              title: '成功',
        ...
        let msg = "是否推荐该条信息?";
        let obj = {"id":row.id ,"isRecommend":1};
        if(row.isRecommend==1){
               msg = "是否取消推荐该条信息？";
               obj = {"id":row.id ,"isRecommend":0};
        }
        self.messageBox(msg,()=>{
          fetch("/supply/update",obj, "PUT").then(
              res => {
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/supply/del/"+ row.id, row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
        ...
        self.viewData["sourceType"] = 2;
        self.viewData["introduce"]=row.introduce;
      } 
    },
    loadData(){
      let self = this;
     ...
     debugger
      ...
      fetch("/supply/getpage2/"+uid+"/0/0/0/"+(self.currentPage )+"/"+self.pageSize,{title:self.keyWords,status:self.status}, "post").then(
        res => {
    ...
    deleteD(){
      ...
      self.messageBox("此操作将永久删除选中记录, 是否继续?",()=>{
        debugger
        fetch("/supply/del/"+param, {}, "delete").then(
          res => {
            this.loadData();
            this.$notify({
            title: '成功',
