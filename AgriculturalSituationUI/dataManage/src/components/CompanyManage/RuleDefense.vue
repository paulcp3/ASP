  ...
  <div class="company-msg box-padding customer-manage">
    ...
    <div class="table-contair">
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="hits" label="Hits"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/unifyingcontrol/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
    ...
    loadData(){
      ...
      let self = this;
      fetch("/unifyingcontrol/getpage2/"+uid+"/"+(self.currentPage )+"/"+self.pageSize,{title:self.keyWords,status:self.status}, "post").then(
        res => {
    ...
    deleteD(){
      ...
      self.messageBox("此操作将永久删除选中记录, 是否继续?",()=>{
        debugger
        fetch("/unifyingcontrol/del/"+param, {}, "delete").then(
          res => {
            this.loadData();
            this.$notify({
            title: '成功',
    ...
    updateOrSave(){
      if(this.Dtitle=="编辑"){
        fetch("/unifyingcontrol/update",this.formEditer, "PUT").then(
        res => {
        this.loadData();
        ...
        debugger
        this.formEditer["uuid"] = "24acfc1215da435cbe76c69beb07f32b";
        fetch("/unifyingcontrol/add",this.formEditer, "POST").then(
        res => {
        this.loadData();
        this.dialogEditerVisible=false;
        this.$notify({
