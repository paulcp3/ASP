  ...
  <div class="company-msg box-padding customer-manage">
    ...
    <div class="table-contair">
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <!-- <el-table-column prop="uRegionId" label="注册名称"></el-table-column> -->
        ...
        <el-table-column prop="need" label="采购项目"></el-table-column>
    ...
    <el-dialog :title="Dtitle" :visible.sync="dialogContentVisible" center width="50%">
      <el-form :model="viewData" label-width="100px">
            ...
            <quill-editor v-model="viewData.need"
                    ref="myQuillEditor"
                    class="editer"
        ...
        <!-- <el-input
            ...
            v-model="viewData.need" autocomplete="off">
          </el-input> -->
      </el-form-item> 
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 3) {
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/seekpurchase/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
        ...
        self.viewData["uuid"] = Cookies.get('uuid');
                self.viewData["need"] = row.need;
        self.viewData["sourceType"] = 1;
      } 
    },
    loadData(){
      var uid =  Cookies.get('uuid')
      let self = this;
      fetch("/seekpurchase/getpage2/"+uid+"/"+(self.currentPage)+"/"+self.pageSize,{title:self.keyWords,status:self.status}, "post").then(
        res => {
    ...
    deleteD(){
      ...
      self.messageBox("此操作将永久删除选中记录, 是否继续?",()=>{
        debugger
        fetch("/seekpurchase/del/"+param, {}, "delete").then(
          res => {
            this.loadData();
            this.$notify({
            title: '成功',
