  ...
  <div class="company-msg box-padding customer-manage">
    ...
    <div class="table-contair">
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="agorae" label="区域"></el-table-column>
        <el-table-column prop="updateTime" label="更新时间"></el-table-column>
        <el-table-column prop="wholesalePrice" label="价格"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/productionprice/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
                title: '成功',
    ...
    // approved(){
    //   let self = this;
    //   fetch("/auditing/add", self.ReviewData ,"POST").then(
    //     res => {
    //       this.loadData();
    //       this.$notify({
    //         title: '成功',
    ...
    loadData(){
      let self = this;
      fetch("/productionprice/getpage/"+( self.currentPage -1 )+"/"+self.pageSize,{}, "GET").then(
        res => {
    ...
    deleteD(){
      ...
      self.messageBox("此操作将永久删除选中记录, 是否继续?",()=>{
        debugger
        fetch("/productionprice/del/"+ param, {}, "delete").then(
          res => {
            this.loadData();
            this.$notify({
              title: '成功',
