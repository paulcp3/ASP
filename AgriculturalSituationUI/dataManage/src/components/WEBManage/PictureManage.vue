  ...
  <div class="company-msg box-padding customer-manage">
    ...
    <div class="table-contair">
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%">
        ...
        <el-table-column prop="imageTitle" label="图片标题"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/supplyimage/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
    ...
    loadData(){
      let self = this;
      fetch("/supplyimage/getpage/"+( self.currentPage -1 )+"/"+self.pageSize,{}, "GET").then(
        res => {
    ...
    updateOrSave(){
      if(this.Dtitle=="编辑"){
        fetch("/supplyimage/update",this.formEditer, "POST").then(
        res => {
        this.loadData();
        ...
        fetch("/supplyimage/add",this.formEditer, "PUT").then(
        res => {
        this.loadData();
        this.dialogEditerVisible=false;
        this.$notify({
