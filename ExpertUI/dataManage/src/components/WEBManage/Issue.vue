<template>
  <div class="company-msg box-padding customer-manage">
    <title-bar name="病虫害预警"></title-bar>
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/alarm/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
    ...
    loadData(){
      ...
      fetch("/alarm/getpage2/"+ self.uuid + '/' + ( self.currentPage  )+"/"+self.pageSize+"?key="+self.keyWords ,{}, "GET").then(
        res => {
    ...
    updateOrSave(){
      if(this.Dtitle=="编辑"){
        ...
        fetch("/alarm/update",this.formEditer, "PUT").then(
        res => {
        this.loadData();
        ...
        this.formEditer.content = delHtmlTag(this.formEditer.content);
        fetch("/alarm/add",this.formEditer, "POST").then(
        res => {
        this.loadData();
        this.dialogEditerVisible=false;
        this.$notify({
