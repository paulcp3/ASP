    ...
    <el-dialog :title="Dtitle" :visible.sync="dialogEditerVisible" center width="50%">
      <el-form :model="formEditer" label-width="100px">
        ...
        <el-form-item label="ID">
          <el-input v-model="formEditer.questionId" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleClick(row, type) {
      ...
      if (type === 1){
        ...
        self.messageBox("此操作将永久删除该条记录, 是否继续?",()=>{
          fetch("/answer/del/"+ row.id,row, "delete").then(
            res => {
              this.loadData();
              this.$notify({
              title: '成功',
    ...
    loadData(){
      let self = this;
      fetch("/answer/getpage/"+( self.currentPage )+"/"+self.pageSize,{}, "GET").then(
        res => {
    ...
    updateOrSave(){
      if(this.Dtitle=="编辑"){
        fetch("/answer/update",this.formEditer, "PUT").then(
        res => {
        this.loadData();
        ...
        fetch("/answer/add",this.formEditer, "POST").then(
        res => {
        this.loadData();
        this.dialogEditerVisible=false;
        this.$notify({
