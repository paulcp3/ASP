      ...
      <el-table :data="tableData" stripe :height="setHeights-170" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="type" label="用户状态">
          <template slot-scope="scope">
            ...
            <el-tag v-else-if="scope.row.type == 4">
              普通用户
            </el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadData(){
      ...
      fetch("/user/getpage2/" +self.uuid +"/"+self.currentPage+"/"+self.pageSize+"?key="+self.keyWords,{}, "GET").then(
        res => {
    ...
    updateOrSave(){
      ...
      if(this.Dtitle=="添加"){
        ...
        fetch("/user/update2",this.formEditer, "PUT").then(
        res => {
        this.loadData();
        this.dialogEditerVisible=false;
        this.$notify({
